...
void parsingInput(string inputString, vector <content> &infix)
{
	...
	for(int i=0; i<numbers.size(); i++)
	{
		if(numbers[i]== ' ')
		{
			...
			{
				//if we get space in 0 num index
				content node;
				makeContentWithSymbol(node, inputString[i]);
				infix.push_back(node);
			}
...
void printPostfix(vector <content> postfix)
{
	cout << "Postfix Expression : "<< " ";
	for(int i=0; i<postfix.size(); i++)
	{
		if(postfix[i].flag) cout << postfix[i].number << " , ";
		else cout << postfix[i].symbol << " , ";
	}
	cout << endl;
}
...
void removeSpace(string &inputString) 
{
	
	string temp = ""; // inititally empty
	
	for( int i=0; i<inputString.size(); i++) 
	{
		if( inputString[i] != ' ' ) 
		{
			temp = temp + inputString[i]; // add to temp if current character is not space
		}
	}
	
	inputString = temp; // asisgn temp to line now 
}
